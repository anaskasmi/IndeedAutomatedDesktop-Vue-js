<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateItemDialog />
    <v-card-title style="color: #30475e">Experiences Items</v-card-title>
...
<script>
import EditableText from "@/views/description-builder/experiences/components/items/Editable.vue";
import CreateItemDialog from "@/views/description-builder/experiences/components/items/CreateItem.vue";
export default {
  created() {
    this.fetchItems();
  },
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderExperiencesModule/deleteExperienceItem",
                item._id
              );
              this.$swal.fire({
