<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateSetDialog />
    <v-card-title style="color: #30475e">Compensations Sets</v-card-title>
      ...
      >
        ...
        <template v-slot:expanded-item="{ item }">
          <div class="my-6" v-if="theSetUnderUpdate != item._id">
            <v-chip
              v-for="compensation in item.compensations"
              class="ml-4 my-1"
              :key="compensation._id"
              :color="itemIdUnderDelete == compensation._id ? 'red' : 'info'"
              close
              :small="itemIdUnderDelete != compensation._id"
              :medium="itemIdUnderDelete == compensation._id"
              :close-icon="
                itemIdUnderDelete == compensation._id ? 'mdi-close' : 'mdi-delete'
              "
              text-color="white"
              @click:close="deleteItemFromSet(item._id, compensation._id)"
            >
              {{ compensation.name }}
            </v-chip>
            <v-btn
...
<script>
import EditableText from "@/views/description-builder/compensations/components/sets/Editable.vue";
import CreateSetDialog from "@/views/description-builder/compensations/components/sets/CreateSet.vue";
export default {
  created() {
    this.fetchSets();
  },
  ...
  methods: {
    ...
    async updateSet(set) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderCompensationsModule/updateSet",
          set
        );
    ...
    async deleteItemFromSet(setId, itemId) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderCompensationsModule/deleteItemFromSet",
          { setId, itemId }
        );
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderCompensationsModule/deleteSet",
                set._id
              );
              this.$swal.fire({
