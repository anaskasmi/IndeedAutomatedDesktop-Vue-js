<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateItemDialog />
    <v-card-title style="color: #30475e">Compensations Items</v-card-title>
...
<script>
import EditableText from "@/views/description-builder/compensations/components/items/Editable.vue";
import CreateItemDialog from "@/views/description-builder/compensations/components/items/CreateItem.vue";
export default {
  created() {
    this.fetchItems();
  },
    ...
    createItemDialogVisible: {
      get: function () {
        return this.$store.getters[
          "DescriptionBuilderCompensationsModule/createItemDialogVisible"
        ];
      },
      set: function (newVal) {
        this.$store.commit(
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderCompensationsModule/deleteCompensationItem",
                item._id
              );
              this.$swal.fire({
