<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateSetDialog />
    <v-card-title style="color: #30475e">Incentives Sets</v-card-title>
      ...
      >
        ...
        <template v-slot:expanded-item="{ item }">
          <div class="my-6" v-if="theSetUnderUpdate != item._id">
            <v-chip
              v-for="incentive in item.incentives"
              class="ml-4 my-1"
              :key="incentive._id"
              :color="itemIdUnderDelete == incentive._id ? 'red' : 'info'"
              close
              :small="itemIdUnderDelete != incentive._id"
              :medium="itemIdUnderDelete == incentive._id"
              :close-icon="
                itemIdUnderDelete == incentive._id ? 'mdi-close' : 'mdi-delete'
              "
              text-color="white"
              @click:close="deleteItemFromSet(item._id, incentive._id)"
            >
              {{ incentive.name }}
            </v-chip>
            <v-btn
...
<script>
import EditableText from "@/views/description-builder/incentives/components/sets/Editable.vue";
import CreateSetDialog from "@/views/description-builder/incentives/components/sets/CreateSet.vue";
export default {
  created() {
    this.fetchSets();
  },
    ...
    createSetDialogVisible: {
      get: function () {
        return this.$store.getters[
          "DescriptionBuilderIncentivesModule/createSetDialogVisible"
        ];
      },
      set: function (newVal) {
        this.$store.commit(
  ...
  methods: {
    ...
    async updateSet(set) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderIncentivesModule/updateSet",
          set
        );
    ...
    async deleteItemFromSet(setId, itemId) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderIncentivesModule/deleteItemFromSet",
          { setId, itemId }
        );
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderIncentivesModule/deleteSet",
                set._id
              );
              this.$swal.fire({
