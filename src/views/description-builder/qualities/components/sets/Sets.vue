<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateSetDialog />
    <v-card-title style="color: #30475e">Qualities Sets</v-card-title>
      ...
      >
        ...
        <template v-slot:expanded-item="{ item }">
          <div class="my-6" v-if="theSetUnderUpdate != item._id">
            <v-chip
              v-for="experience in item.qualities"
              class="ml-4 my-1"
              :key="experience._id"
...
<script>
import EditableText from "@/views/description-builder/qualities/components/sets/Editable.vue";
import CreateSetDialog from "@/views/description-builder/qualities/components/sets/CreateSet.vue";
export default {
  created() {
    this.fetchSets();
  },
    ...
    createSetDialogVisible: {
      get: function () {
        return this.$store.getters[
          "DescriptionBuilderQualitiesModule/createSetDialogVisible"
        ];
      },
      set: function (newVal) {
        this.$store.commit(
  ...
  methods: {
    ...
    async updateSet(set) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderQualitiesModule/updateSet",
          set
        );
    ...
    async deleteItemFromSet(setId, itemId) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderQualitiesModule/deleteItemFromSet",
          { setId, itemId }
        );
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderQualitiesModule/deleteSet",
                set._id
              );
              this.$swal.fire({
