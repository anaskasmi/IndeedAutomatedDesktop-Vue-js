...
<script>
import EditableText from "@/views/description-builder/apply-methods/components/items/Editable.vue";
import CreateItemDialog from "@/views/description-builder/apply-methods/components/items/CreateItem.vue";
export default {
  created() {
    this.fetchItems();
  },
    ...
    createItemDialogVisible: {
      get: function () {
        return this.$store.getters[
          "DescriptionBuilderApplyMethodsModule/createItemDialogVisible"
        ];
      },
      set: function (newVal) {
        this.$store.commit(
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderApplyMethodsModule/deleteApplyMethodItem",
                item._id
              );
              this.$swal.fire({
