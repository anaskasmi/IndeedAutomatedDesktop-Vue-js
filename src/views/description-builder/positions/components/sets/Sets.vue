<template>
  <v-card :loading="isLoading" :disabled="isLoading">
    <CreateSetDialog />
    <v-card-title style="color: #30475e">Positions Sets</v-card-title>
        ...
        <template v-slot:expanded-item="{ item }">
          <div class="my-6" v-if="theSetUnderUpdate != item._id">
            <v-chip
              ...
              class="ml-4 my-1"
              :key="position._id"
              :color="itemIdUnderDelete == position._id ? 'red' : 'info'"
              close
              :small="itemIdUnderDelete != position._id"
              :medium="itemIdUnderDelete == position._id"
              :close-icon="
                itemIdUnderDelete == position._id ? 'mdi-close' : 'mdi-delete'
              "
              text-color="white"
              @click:close="deleteItemFromSet(item._id, position._id)"
            >
              {{ position.name }}
            </v-chip>
            <v-btn
...
<script>
import EditableText from "@/views/description-builder/positions/components/sets/Editable.vue";
import CreateSetDialog from "@/views/description-builder/positions/components/sets/CreateSet.vue";
export default {
  created() {
    this.fetchSets();
  },
  ...
  methods: {
    ...
    async deleteItemFromSet(setId, itemId) {
      ...
      try {
        await this.$store.dispatch(
          "DescriptionBuilderPositionsModule/deleteItemFromSet",
          { setId, itemId }
        );
      ...
      this.$swal
        ...
        .then(async (result) => {
          if (result.isConfirmed) {
            ...
            try {
              await this.$store.dispatch(
                "DescriptionBuilderPositionsModule/deleteSet",
                set._id
              );
              this.$swal.fire({
