...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters.getRepostingSteps.fillIn_webSite;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters.getFailureMsgs.fillIn_webSite;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("execute_fillIn_webSite");
    },
  },
};
</script>
