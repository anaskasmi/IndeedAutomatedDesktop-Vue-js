...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters.getRepostingSteps.fillIn_paymentType;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters.getFailureMsgs.fillIn_paymentType;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("execute_fillIn_paymentType");
    },
  },
};
</script>