...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters.getRepostingSteps.fillIn_paymentPer;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters.getFailureMsgs.fillIn_paymentPer;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("execute_fillIn_paymentPer");
    },
  },
};
</script>
