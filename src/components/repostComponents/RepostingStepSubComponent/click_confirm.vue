...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters['repostPageModule/getRepostingSteps'].click_confirm;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters['repostPageModule/getFailureMsgs'].click_confirm;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("repostPageModule/execute_click_confirm");
    },
  },
};
</script>
