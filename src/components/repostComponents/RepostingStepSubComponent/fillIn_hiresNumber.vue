...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters['repostPageModule/getRepostingSteps'].fillIn_hiresNumber;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters['repostPageModule/getFailureMsgs'].fillIn_hiresNumber;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("repostPageModule/execute_fillIn_hiresNumber");
    },
  },
};
</script>
