...
export default {
  ...
  computed: {
    status: {
      get() {
        return this.$store.getters['repostPageModule/getRepostingSteps'].getJobFullDetails;
      },
    },
    failureMsg: {
      get() {
        return this.$store.getters['repostPageModule/getFailureMsgs'].getJobFullDetails;
      },
    },
  },
  methods: {
    execute() {
      this.$store.dispatch("repostPageModule/execute_getJobFullDetails");
    },
  },
};
</script>