...
export default {
  ...
  methods: {
    async fillInPage() {
      this.isLoading = true;
      try {
        await this.$store.dispatch("execute_fillIn_isJobFullTimeOrPartTime");
        await this.$store.dispatch("execute_fillIn_schedule");
        await this.$store.dispatch("execute_fillIn_hiresNumber");
        await this.$store.dispatch("execute_fillIn_deadline");
